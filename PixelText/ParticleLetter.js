...
export class ParticleLetter {
	constructor(x, y, mouse, settings) {
		this.x = x
		this.y = y
		this.currentX = x + getRandom(-100, 100)
		this.currentY = y + getRandom(-100, 100)
		this.size = settings.size * settings.scale
	...
	update() {
		...
		if (distance < this.mouse.radius && this.mouse.isActive) {
			...
			if (Math.abs(this.currentX - this.x) > this.size + this.gap) {
				this.currentX += Math.floor(forceX * this.speed * this.weight)
			} else {
				this.currentX = this.x
			}

			if (Math.abs(this.currentY - this.y) > this.size + this.gap) {
